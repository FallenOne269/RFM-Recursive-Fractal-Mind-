...
from utils import PluginRegistry, load_config, sanitize_path, validate_task
...
class RecursiveFractalMind:
    """Lightweight orchestrator that wires the core subsystems together."""

    _PLUGIN_MODULES = (
        "fractal_engine.engine",
        "swarm_coordinator.coordinator",
        "quantum_processor.processor",
        "meta_learner.meta",
    )

    def __init__(self, config_path: str | None = None, *, registry: PluginRegistry | None = None) -> None:
        self._config = load_config(config_path)
        self._registry = registry or PluginRegistry()
        self._registry.load_plugins(self._PLUGIN_MODULES)

        self._fractal = self._registry.create("fractal_engine", config=self._config.fractal)
        self._swarm = self._registry.create("swarm_coordinator", config=self._config.swarm)
        self._quantum = self._registry.create("quantum_processor", config=self._config.quantum)
        self._meta = self._registry.create("meta_learner", config=self._config.meta)

        self._history: list[dict[str, object]] = []
        self._last_result: dict[str, object] | None = None
        self._cycles = 0

    def run_cycle(self, task: Dict[str, Any]) -> Dict[str, object]:
        """Validate and process a task through all components."""

        validate_task(task)
        task_id = str(task.get("id", f"task-{self._cycles + 1}"))
        start = time.perf_counter()

        fractal_output = self._fractal.run(task["payload"])
        swarm_output = self._swarm.coordinate(float(task["complexity"]), [0.0] * self._config.swarm.swarm_size)
        quantum_output = self._quantum.run(fractal_output.aggregated_signal, float(task["complexity"]))

        performance_score = float(
            0.4 * swarm_output.overall_success_rate
            + 0.4 * quantum_output.confidence
            + 0.2 * max(0.0, 1.0 - float(task["complexity"]))
        )
        ...
        result: Dict[str, object] = {
            ...
            "quantum_output": quantum_output.to_dict(),
    ...
    def get_status(self) -> Dict[str, object]:
        """Return high level orchestrator status."""
        ...
        return {
            ...
            "components": {
                "fractal_engine": self._fractal is not None,
                "swarm_coordinator": self._swarm is not None,
                "quantum_processor": self._quantum is not None,
                "meta_learner": self._meta is not None,
            },
        }

    def save_state(self, path: str | os.PathLike[str] | None = None) -> str:
        """Persist orchestrator history to disk."""

        state_dir = sanitize_path(self._config.persistence.state_dir, allow_external=True)
        Path(state_dir).mkdir(parents=True, exist_ok=True)
        target = Path(path) if path else Path(state_dir) / "rfai_state.json"
        ...
        document = {
            "metadata": {
                "version": self._config.persistence.version,
    ...
    def load_state(self, path: str | os.PathLike[str]) -> None:
        """Restore orchestrator history from disk."""

        state_path = sanitize_path(path, allow_external=True)
        data = json.loads(Path(state_path).read_text(encoding="utf-8"))
        payload = data.get("payload", {})
        self._history = list(payload.get("history", []))
        self._last_result = payload.get("last_result")
        self._cycles = int(payload.get("cycle_count", len(self._history)))
...
__all__ = ["RecursiveFractalMind", "RecursiveFractalAutonomousIntelligence"]

...
from RFAI_System_Package.src.rfai_system import (
...
def _load_config() -> Dict[str, Any]:
    ...
    logger.warning(
        "Configuration file %s not found; falling back to built-in defaults.",
        config_path,
    )
    return {
        "max_fractal_depth": 4,
...
class TaskInput(BaseModel):
    ...
    task_type: str = Field(..., description="Task archetype or intent descriptor")
...
async def process_task(task: TaskInput) -> Dict[str, Any]:
    ...
    core_task["type"] = core_task.pop("task_type")
